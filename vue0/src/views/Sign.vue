    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"
      ...
      <el-table-column label="考试名称">
        <template v-slot="scope">
          <span v-if="exams && exams.length">{{ exams.find(v => v.id === scope.row.examId) ? exams.find(v => v.id ===
            scope.row.examId).name : '' }}</span>
        </template>
      </el-table-column>
      <el-table-column label="学生">
        <template v-slot="scope">
          <span v-if="users && users.length">{{ users.find(v => v.id === scope.row.userId) ? users.find(v => v.id ===
            scope.row.userId).nickname : '' }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="state" label="审核状态"></el-table-column>
      <el-table-column label="审核" width="240">
        <template v-slot="scope">
          <el-button type="success" v-if="scope.row.state === '待审核'"
            @click="changeState(scope.row, '审核通过')">审核通过</el-button>
          <el-button type="danger" v-if="scope.row.state === '待审核'"
            @click="changeState(scope.row, '审核不通过')">审核不通过</el-button>
        </template>
      </el-table-column>
