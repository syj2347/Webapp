  ...
  <div>
      ...
      <el-table-column label="考卷">
        <template v-slot="scope">
          <div style="display: flex;align-items: center;">
            <el-button type="primary" @click="viewPaper1(scope.row.id)">查看考卷</el-button>
            <el-button type="warning" @click="viewPaper2(scope.row.id)">*</el-button>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="380" align="center">
        <template slot-scope="scope">
          <el-button type="primary" plain @click="handlePaper(scope.row.id, scope.row.courseId)">手动组卷 <i
              class="el-icon-document"></i></el-button>
          <el-button type="primary" @click="takePaper(scope.row.id, scope.row.courseId)">自动组卷 <i
              class="el-icon-document"></i></el-button>
    ...
    <el-dialog title="手动组卷" :visible.sync="dialogFormVisible3" width="55%" :close-on-click-modal="false">
      <div style="width: 90%; padding-left: 120px">
        <el-transfer :titles="['题库', '已选']" filter-placeholder="请输入类型：1选择，2判断，3问答" filterable
          :filter-method="filterMethod" v-model="form2.handleQuestionIds" :data="paperQuestions"
    ...
    <el-dialog title="查看考卷（含答案）" :visible.sync="dialogFormVisible2" width="60%" :close-on-click-modal="false">
      <el-card ref="content1">
      ...
      <el-button type="primary" @click="exportToWord1">导出文档<i class="el-icon-bottom"></i></el-button>
    </el-dialog>

    <el-dialog title="查看考卷（不含答案）" :visible.sync="dialogFormVisible4" width="60%" :close-on-click-modal="false">
      <el-card ref="content2">
        <div v-for="(item, index) in questiones" :key="item.id" style="margin: 20px 0">
          <div style="margin: 10px 0; font-size: 20px"><span>{{ index + 1 }}.</span> {{ item.name }}
            <span style="font-size: 14px" v-if="item.type === 1">（选择题）</span>
            <span style="font-size: 14px" v-if="item.type === 2">（判断题）</span>
      ...
      <div style="margin-bottom: 10px;"></div>
      <el-button type="primary" @click="exportToWord2">导出文档<i class="el-icon-bottom"></i></el-button>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    exportToWord1() {
      let content = this.$refs.content1.$el.outerHTML;  // 获取要导出的内容
      let converted = htmlDocx.asBlob(content);  // 将 HTML 转换为 Word 文档
      saveAs(converted, 'output_with_ans.docx');  // 保存 Word 文档
    },
    exportToWord2() {
      let content = this.$refs.content2.$el.outerHTML;  // 获取要导出的内容
      let converted = htmlDocx.asBlob(content);  // 将 HTML 转换为 Word 文档
      saveAs(converted, 'output_no_ans.docx');  // 保存 Word 文档
    },
  }
}
</script>
